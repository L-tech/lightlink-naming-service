...
const Domain = () => {
  ...
  const handleSearch = () => {
    const containsLinkSubstr =
      searchQuery.slice(searchQuery.length - 3, searchQuery.length) === 'link'
    navigate(
      `/${
        isValidAddress(searchQuery)
          ? `address/${searchQuery}/registrant`
          : `domain/${searchQuery}${containsLinkSubstr ? '' : '.link'}/register`
      }`,
    )
  }

  const handleViewOnExplorer = () => {
    window.open(`https://hyperspace.linkfox.info/en/address/${searchQuery}`)
  }
